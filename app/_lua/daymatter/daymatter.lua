...
local function createWatchface(parent)
    ...
    --倒数日方框
    t.countdown1 = Image(wfRoot, imgPath("before.bin"), { 6, 59 })
    t.countdown2 = Image(wfRoot, imgPath("later.bin"), { 6, 247 })

    -- 电量
    t.chargeCont = Label(wfRoot, "100%", { w = 84, h = 27, x = 54, y = 450 }, "#ffffff")

    -- 倒计时天数
    t.timeDay = imageGroup(wfRoot, { 0, 94 })
    t.timeDayChild1 = t.timeDay:setChild(imgPath("0.bin"), { x = 15 })
    t.timeDayChild2 = t.timeDay:setChild(imgPath("7.bin"), { x = 55 })
    t.timeDayChild3 = t.timeDay:setChild(imgPath("2.bin"), { x = 96 })
    t.timeDayChild4 = t.timeDay:setChild(imgPath("1.bin"), { x = 136 })
    t.timeDayChild5 = t.timeDay:setChild(imgPath("today.bin"), { x = 16, y = 4 })

    -- 小时分钟
    t.timeHourHigh = Image(wfRoot, imgPath("0.bin"), { 5.5, 282 })
    t.timeHourLow = Image(wfRoot, imgPath("9.bin"), { 46.5, 282 })
    t.timeGang = Image(wfRoot, imgPath("mao.bin"), { 75.5, 282 })
    t.timeMinuteHigh = Image(wfRoot, imgPath("2.bin"), { 104.5, 282 })
    t.timeMinuteLow = Image(wfRoot, imgPath("8.bin"), { 145.5, 282 })


    -- 文字
    t.gaokao = Label(wfRoot, "", { w = 180, h = 27, x = 6, y = 76.5 }, "#ffffff")
    t.countdown = Label(wfRoot, "目标日: 2026-6-8", { w = 180, h = 27, x = 6, y = 200.5 }, "#ffffff")

    -- 日期
    t.dateCont = Label(wfRoot, "03/08 周六", { w = 155, h = 27, x = 19, y = 388.5 }, "#ffffff")
...
local function uiCreate()
    ...
    dataman.subscribe("timeSecondLow", watchface.timeDay.widget, function(obj, value)
        ...
        elseif days < 1000 then
            ...
            watchface.timeDayChild3:clear_flag(lvgl.FLAG.HIDDEN)
            src = string.format("%d.bin", days % 10)
            watchface.timeDayChild4:set({ src = imgPath(src), x = 136 })
            watchface.timeDayChild4:clear_flag(lvgl.FLAG.HIDDEN)
        end
    end)
